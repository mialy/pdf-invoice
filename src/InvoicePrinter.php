...
class InvoicePrinter extends tFPDF
{
    ...
    public $currencyAfterPrice = true;
    ...
    public function addItem($item, $description = "", $quantity, $vat, $price, $discount = 0, $total)
    {
        ...
        if ($vat !== false) {
            $p['vat'] = $vat;
            if (is_numeric($vat)) {
                $p['vat'] = $this->getAmountFormatted($vat);
            }
            $this->vatField = true;
            $this->recalculateColumns();
        }
        ...
        if ($discount !== false) {
            ...
            if (is_numeric($discount)) {
                $p['discount'] = $this->getAmountFormatted($discount);
            }
    ...
    public function addTotal($name, $value, $colored = false)
    {
        ...
        $t['value'] = $value;
        if (is_numeric($value)) {
            $t['value'] = $this->getAmountFormatted($value);
        }
        $t['colored']   = $colored;
        $this->totals[] = $t;
    }
    ...
    public function Header()
    {
        ...
        if(isset($this->title) and !empty($this->title)) {
            $this->Cell(0, 5, mb_strtoupper($this->title, self::ICONV_CHARSET_INPUT), 0, 1, 'R');
        }
        ...
        if (!empty($this->time)) {
            ...
            $this->SetFont($this->font, 'B', 9);
            $this->SetTextColor($this->color[0], $this->color[1], $this->color[2]);
            $this->Cell(32, $lineheight, mb_strtoupper($this->lang['time'], self::ICONV_CHARSET_INPUT) . ':', 0, 0, 'L');
            $this->SetTextColor(50, 50, 50);
            $this->SetFont($this->font, '', 9);
        ...
        if (!empty($this->due)) {
            ...
            $this->SetFont($this->font, 'B', 9);
            $this->SetTextColor($this->color[0], $this->color[1], $this->color[2]);
            $this->Cell(32, $lineheight, mb_strtoupper($this->lang['due'], self::ICONV_CHARSET_INPUT) . ':', 0, 0, 'L');
            $this->SetTextColor(50, 50, 50);
            $this->SetFont($this->font, '', 9);
        ...
        if ($this->PageNo() == 1) {
            ...
            if ($this->display_tofrom === true) {
                if ($this->displayToFromHeaders === true) {
                    $this->Cell($width, $lineheight, mb_strtoupper($this->lang['from'], self::ICONV_CHARSET_INPUT), 0, 0, 'L');
                    $this->Cell(0, $lineheight, mb_strtoupper($this->lang['to'], self::ICONV_CHARSET_INPUT), 0, 0, 'L');
                    $this->Ln(7);
        ...
        if (!isset($this->productsEnded)) {
            ...
            $this->Cell($this->firstColumnWidth, 10, mb_strtoupper($this->lang['product'], self::ICONV_CHARSET_INPUT), 0, 0, 'L', 0);
            $this->Cell($this->columnSpacing, 10, '', 0, 0, 'L', 0);
            $this->Cell($width_other, 10, mb_strtoupper($this->lang['qty'], self::ICONV_CHARSET_INPUT), 0, 0, 'C', 0);
            if (isset($this->vatField)) {
                $this->Cell($this->columnSpacing, 10, '', 0, 0, 'L', 0);
                $this->Cell($width_other, 10, mb_strtoupper($this->lang['vat'], self::ICONV_CHARSET_INPUT), 0, 0, 'C', 0);
            }
            $this->Cell($this->columnSpacing, 10, '', 0, 0, 'L', 0);
            $this->Cell($width_other, 10, mb_strtoupper($this->lang['price'], self::ICONV_CHARSET_INPUT), 0, 0, 'C', 0);
            if (isset($this->discountField)) {
                $this->Cell($this->columnSpacing, 10, '', 0, 0, 'L', 0);
                $this->Cell($width_other, 10, mb_strtoupper($this->lang['discount'], self::ICONV_CHARSET_INPUT), 0, 0, 'C', 0);
            }
            $this->Cell($this->columnSpacing, 10, '', 0, 0, 'L', 0);
            $this->Cell($width_other, 10, mb_strtoupper($this->lang['total'], self::ICONV_CHARSET_INPUT), 0, 0, 'C', 0);
            $this->Ln();
    ...
    public function Body()
    {
        ...
        if ($this->items) {
            foreach ($this->items as $item) {
                ...
                $x = $this->GetX();
                $this->Cell($this->firstColumnWidth, $cHeight, $item['item'], 0, 0, 'L',
                    1);
                if ($item['description']) {
                    $resetX = $this->GetX();
                    $resetY = $this->GetY();
                ...
                $this->Cell($this->columnSpacing, $cHeight, '', 0, 0, 'L', 0);
                $this->Cell($width_other, $cHeight, $this->getAmountFormatted($item['price']), 0, 0, 'C', 1);
                if (isset($this->discountField)) {
                    $this->Cell($this->columnSpacing, $cHeight, '', 0, 0, 'L', 0);
                ...
                $this->Cell($this->columnSpacing, $cHeight, '', 0, 0, 'L', 0);
                $this->Cell($width_other, $cHeight, $this->getAmountFormatted($item['total']), 0, 0, 'C', 1);
                $this->Ln();
        ...
        if ($this->badge) {
            ...
            $this->Write(10, mb_strtoupper($badge, self::ICONV_CHARSET_INPUT));
            $this->Rotate(0);
        ...
        foreach ($this->addText as $text) {
            if ($text[0] == 'title') {
                $this->SetFont($this->font, 'b', 9);
                $this->SetTextColor(50, 50, 50);
                $this->Cell(0, 10, mb_strtoupper($text[1], self::ICONV_CHARSET_INPUT), 0, 0, 'L', 0);
                $this->Ln();
                $this->SetLineWidth(0.3);
                $this->SetDrawColor($this->color[0], $this->color[1], $this->color[2]);
    ...
    public function getAmountFormatted($price) {
        $formattedPrice =  number_format($price, 2, $this->referenceformat[0], $this->referenceformat[1]);

        if ($this->currencyAfterPrice) {
            return $formattedPrice . ' ' . $this->currency;
        } else {
            return $this->currency . ' ' . $formattedPrice;
        }
    }
}
